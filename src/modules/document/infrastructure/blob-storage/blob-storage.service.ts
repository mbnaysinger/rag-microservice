...
export class BlobStorageService implements IDocumentStoragePort {
  ...
  constructor(private readonly configService: ConfigServerService) {
    const connectionString = this.configService.get('config.blobstorage.connection-string');
  ...
  async uploadFile(file: Express.Multer.File, blobName: string): Promise<string> {
    try {
      ...
      this.logger.error(`Error uploading file to Azure Blob Storage: ${error.message}`);
